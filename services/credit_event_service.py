"""Persistent free credit event creation and one-time user claims."""
        ...
        try:
            ...
            conn.execute(
                """INSERT INTO account_entitlements
                   ...
                   VALUES (?, ?, 0, 'pending', 'credit_event', ?, ?, ?)""",
                (telegram_id, amount, f"{amount} Event Credits", now, now),
            )
            conn.execute(
            ...
            audit_log.log(
                ...
                actor_name=username,
                details=f"Event ID: {event_id}, Credits: {amount}, New balance: {new_balance}",
                result="SUCCESS",
            )
            return {"success": True, "status": "claimed", "event_id": event_id, "amount": amount, "balance": new_balance, "claimed_at": now}
        except Exception as exc:
